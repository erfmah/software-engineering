...
export class CartService {
	...
	async getCartItems(cart): Promise<CartDetails[]> { 
		const cds = await this.cartDetailsRepository.find({where:{cart: cart}, relations: ["product", "product.images"] });
		return cds;
	}
	async findActiveCarts(user): Promise<Cart> {
		const now = new Date()
		const carts = await this.cartRepository.createQueryBuilder("cart")
			.where("cart.end > :now ", { now: now })
			.andWhere("active = 1")
			.andWhere("ordered = 0")
			.andWhere("userId = :id ", { id: user })
	...
	async deleteFromCart(product, user): Promise<CartDetails> {
	...
	async addToCart(product, user): Promise<CartDetails> {
		...
		const foundedProduct = await this.productRepository.findOne({ id: product })
		if (cart) {
			const cartDetails = await this.cartDetailsRepository.find({where: { cart, product }, relations:['cart']})
	...
	async createCart(user): Promise<Cart> {
		const carts = await this.findActiveCarts(user)
		if (carts) {
			return carts;
		} else {
			const cart = new Cart()
